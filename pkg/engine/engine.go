...
import (
	...
	protocolbackends "OpenCNC_config_service/pkg/protocolbackends"
	"OpenCNC_config_service/pkg/structures/topology"
	"OpenCNC_config_service/pkg/structures/topology_config"
)
...
func (m *MappingEngine) ApplyConfiguration(
	...
	for _, node := range topo.Nodes {
		...
		nodeCfg := m.findNodeConfig(cfg, node.NodeConfigId)
		if nodeCfg == nil {
			continue
		}
		...
		backend.MapAndPush(nodeCfg, *node.ManagementInfo)
	}
...
func (m *MappingEngine) findNodeConfig(
	...
	for _, nodeCfg := range cfg.GetNodeConfigs() {
		if nodeCfg != nil && nodeCfg.GetConfigId() == *nodeConfigID {
			return nodeCfg
		}
	}
	return nil
